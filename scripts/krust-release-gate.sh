...
require_doc_line README.md "M14-M85"
...
require_doc_line docs/krust-milestones.md "done: KrustBoot compact payload identity is \`KRUSTBOOTM85\` version 15"
...
require_doc_line kernel/krust/README.md "M14-M85"
...
mkdir -p "$LOG_DIR"
release_profile="$LOG_DIR/m85-release-profile.txt"
...
cat "$release_profile"
grep -Fq "krustboot=Manifest v1 compact KRUSTBOOTM85 version 15" "$release_profile" || fail "release profile missing M85 KrustBoot identity"
...
echo
echo "Krust release gate ok: clean-clone M14-M85 substrate proof is repeatable."
