...
require_doc_line README.md "M14-M84"
...
require_doc_line docs/krust-milestones.md "done: KrustBoot compact payload identity is \`KRUSTBOOTM84\` version 14"
...
require_doc_line kernel/krust/README.md "M14-M84"
...
mkdir -p "$LOG_DIR"
release_profile="$LOG_DIR/m84-release-profile.txt"
...
cat "$release_profile"
grep -Fq "krustboot=Manifest v1 compact KRUSTBOOTM84 version 14" "$release_profile" || fail "release profile missing M84 KrustBoot identity"
...
echo
echo "Krust release gate ok: clean-clone M14-M84 substrate proof is repeatable."
